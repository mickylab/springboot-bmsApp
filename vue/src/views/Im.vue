    ...
    <el-row>
      <el-col :span="4">
        <el-card style="width: 300px; height: 300px; color: #333">
          <div style="padding-bottom: 10px; border-bottom: 1px solid #ccc">Online users<span style="font-size: 12px">（Click the bubble to chat）</span></div>
...
export default {
  ...
  methods: {
    send() {
      if (!this.chatUser) {
        this.$message({type: 'warning', message: "Please select a chat partner"})
        return;
      }
      if (!this.text) {
        this.$message({type: 'warning', message: "Please enter chat content"})
      } else {
    ...
    createContent(remoteUser, nowUser, text) {  // 这个方法是用来将 json的聊天消息数据转换成 html的。
      ...
      if (nowUser) { // nowUser 表示是否显示当前用户发送的聊天消息，绿色气泡
        html = "<div class=\"el-row\" style=\"padding: 5px 0\">\n" +
            ...
            "    <img src=\"https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png\" alt=\"\" style=\"object-fit: cover;\">\n" +
            "  </span>\n" +
            "  </div>\n" +
            "</div>";
